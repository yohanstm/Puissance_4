...
public class IA {
     ...
     */
    public IA(String motATrouver, int niveau, int nbErreursMax) {
        super();
     ...
     * @param nbErreursMax le nombre total d'essais autorisés
     */
    public IA(String nomFichier, int longMin, int longMax, int niveau, int nbErreursMax) {
        super();
        this.initMotMystere(motATrouver, niveau, nbErreursMax);
    }
    ...
    private void initMotMystere(String motATrouver, int niveau, int nbErreursMax){
        ...
        if (niveau == IA.EXPERT || niveau == IA.DIFFICILE){
        ...
        for (int i=1; i<motATrouver.length()-1; i++){
            ...
            if (this.niveau == IA.EXPERT || Character.isAlphabetic(lettre)){
        ...
        if (niveau != IA.FACILE){ // dernière lettre révélée
            motCrypte += "*";
            this.nbLettresRestantes += 1;
        }
        else{
